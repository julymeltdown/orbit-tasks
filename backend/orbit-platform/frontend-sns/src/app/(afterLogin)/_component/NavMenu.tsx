"use client";

import style from "./navMenu.module.css";
import { useSelectedLayoutSegment } from "next/navigation";
import Link from "next/link";

export default function NavMenu() {
  const segment = useSelectedLayoutSegment();
  const isActive = (value: string) => segment === value;

  return (
    <>
      <li>
        <Link href="/home">
          <div className={style.navPill}>
            <svg width={26} viewBox="0 0 24 24" aria-hidden="true">
              <g>
                <path d="M12 1.696L.622 8.807l1.06 1.696L3 9.679V19.5C3 20.881 4.119 22 5.5 22h13c1.381 0 2.5-1.119 2.5-2.5V9.679l1.318.824 1.06-1.696L12 1.696zM12 16.5c-1.933 0-3.5-1.567-3.5-3.5s1.567-3.5 3.5-3.5 3.5 1.567 3.5 3.5-1.567 3.5-3.5 3.5z"></path>
              </g>
            </svg>
            <div style={{ fontWeight: isActive("home") ? "bold" : "normal" }}>Home</div>
          </div>
        </Link>
      </li>
      <li>
        <Link href="/search">
          <div className={style.navPill}>
            <svg width={26} viewBox="0 0 24 24" aria-hidden="true">
              <g>
                <path d="M10.25 3.75c-3.59 0-6.5 2.91-6.5 6.5s2.91 6.5 6.5 6.5c1.795 0 3.419-.726 4.596-1.904 1.178-1.177 1.904-2.801 1.904-4.596 0-3.59-2.91-6.5-6.5-6.5zm-8.5 6.5c0-4.694 3.806-8.5 8.5-8.5s8.5 3.806 8.5 8.5c0 1.986-.682 3.815-1.824 5.262l4.781 4.781-1.414 1.414-4.781-4.781c-1.447 1.142-3.276 1.824-5.262 1.824-4.694 0-8.5-3.806-8.5-8.5z"></path>
              </g>
            </svg>
            <div style={{ fontWeight: isActive("search") ? "bold" : "normal" }}>Search</div>
          </div>
        </Link>
      </li>
      <li>
        <Link href="/messages">
          <div className={style.navPill}>
            <svg width={26} viewBox="0 0 24 24" aria-hidden="true">
              <g>
                <path d="M1.998 5.5c0-1.381 1.119-2.5 2.5-2.5h15c1.381 0 2.5 1.119 2.5 2.5v13c0 1.381-1.119 2.5-2.5 2.5h-15c-1.381 0-2.5-1.119-2.5-2.5v-13zm2.5-.5c-.276 0-.5.224-.5.5v2.764l8 3.638 8-3.636V5.5c0-.276-.224-.5-.5-.5h-15zm15.5 5.463l-8 3.636-8-3.638V18.5c0 .276.224.5.5.5h15c.276 0 .5-.224.5-.5v-8.037z"></path>
              </g>
            </svg>
            <div style={{ fontWeight: isActive("messages") ? "bold" : "normal" }}>Messages</div>
          </div>
        </Link>
      </li>
      <li>
        <Link href="/notifications">
          <div className={style.navPill}>
            <svg width={26} viewBox="0 0 24 24" aria-hidden="true">
              <g>
                <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6V11c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5S10.5 3.17 10.5 4v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"></path>
              </g>
            </svg>
            <div style={{ fontWeight: isActive("notifications") ? "bold" : "normal" }}>
              Notifications
            </div>
          </div>
        </Link>
      </li>
      <li>
        <Link href="/profile">
          <div className={style.navPill}>
            <svg width={26} viewBox="0 0 24 24" aria-hidden="true">
              <g>
                <path d="M17.863 13.44c1.477 1.58 2.366 3.8 2.632 6.46l.11 1.1H3.395l.11-1.1c.266-2.66 1.155-4.88 2.632-6.46C7.627 11.85 9.648 11 12 11s4.373.85 5.863 2.44zM12 2C9.791 2 8 3.79 8 6s1.791 4 4 4 4-1.79 4-4-1.791-4-4-4z"></path>
              </g>
            </svg>
            <div style={{ fontWeight: isActive("profile") ? "bold" : "normal" }}>Profile</div>
          </div>
        </Link>
      </li>
    </>
  );
}
