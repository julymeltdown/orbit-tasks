...
public class AuthClient implements AuthClientPort {
    ...
    private final IdentityAccessServiceGrpc.IdentityAccessServiceBlockingStub identityStub;

    public AuthClient(AuthServiceGrpc.AuthServiceBlockingStub stub,
                      IdentityAccessServiceGrpc.IdentityAccessServiceBlockingStub identityStub) {
        this.stub = stub;
    ...
    public List<WorkspaceClaimResponse> listWorkspaceClaims(UUID userId) {
        var response = identityStub.getWorkspaceClaims(GetWorkspaceClaimsRequest.newBuilder()
                .setUserId(userId.toString())
                .build());

        return response.getClaimsList().stream()
                .map(claim -> new WorkspaceClaimResponse(
                        UUID.fromString(claim.getWorkspaceId()),
                        claim.getWorkspaceName(),
                        claim.getRole(),
                        claim.getDefaultWorkspace()))
                .toList();
    }
